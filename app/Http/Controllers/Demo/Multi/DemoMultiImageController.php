    ...
    public function action(Request $request)
    {
        ...
        if ($act == 'editv2') {
            ...
            if($image){
                foreach ($image as $id => $img) {
                    $imgDelete = DemoMultipic::findOrFail($id);
                    unlink($imgDelete->image);
                    $make_name =
                ...
                return redirect()
                ->route('demo.multi.list')
                ->with('error', 'Bạn cần ít nhất upload 1 hình');
            }
        ...
        if (!empty($list_check)) {
            if ($act == 'deletev2') {
                ...
                return redirect()
                    ->route('demo.multi.list')
                    ->with('error', 'Bạn cần chọn tác vụ để thực thi !');
            }
        } else {
            return redirect()
                ->route('demo.multi.list')
                ->with('error', 'Bạn cần chọn phần tử cần thực thi !');
        }
    }
}
